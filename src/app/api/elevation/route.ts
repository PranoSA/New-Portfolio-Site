...
export async function POST(req: NextRequest) {
  ...
  if (!contentType) {
    return NextResponse.json(
      { message: "Bad request, Missin Content Typed" },
      { status: 400 }
    );
  }
  ...
  if (contentType.includes("application/json")) {
    ...
    for (const entry of geoJson.coordinates) {
      if (!entry.lat || !entry.long) {
        return NextResponse.json(
          { message: "Bad request, Invalid GeoJson" },
          { status: 400 }
        );
      }
    }
    ...
    for (const entry of geoJson.coordinates) {
      ...
      const response = await fetch(
        `${elevationAPIURL}?locations=${entry.lat},${entry.long}`
      );
    ...
    if (enrichedGeoJson.coordinates.length < 10) {
    ...
    try {
      const mongodb: MongoClient = await MongoClient.connect(
      ...
      if (!contentType) {
        return NextResponse.json(
          { message: "Bad request, Missing Content type" },
          { status: 400 }
        );
      }
    ...
    */
      ...
      if (contentType.includes("application/json")) {
        ...
        return NextResponse.json(
          { message: "Success", data: enrichedGeoJson },
          { status: 200 }
        );
      ...
      return NextResponse.json(
        { message: "Bad request, Invalid Mime Type" },
        { status: 400 }
      );
    } catch (error) {
      return NextResponse.json(
